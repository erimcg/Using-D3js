(function() {
  ...
  let SandBox = window.SandBox = class {
    ...
    run(code, output, meta) {
      ...
      let scripts = this.output.div.parentNode.getElementsByTagName("script");
      for (let i = 0; i < scripts.length; i++) {
        this.output.div.parentNode.removeChild(scripts[i])
      }

      let pre = this.output.div.parentNode.nextSibling;
      let lang = pre.getAttribute("data-language");

      if (lang === 'html') {

        let [scripts, rem] = this.getScripts(code);
        //let rem = this.runScripts(code, this.output.div);

        let elm = document.createElement('div');
        this.output.div.innerHTML = rem;
        //this.output.div.appendChild(elm);

        for (let elm of scripts) {
          this.output.div.parentNode.appendChild(elm);
        }
        return;
      }
      ...
      this.prepare(code)
        .then(code => {
          ...
          this.win.d3 = window.d3;

          let value = this.win.eval(code)
          //let value = window.eval(code)
          if (this.notify.onRun)
            this.notify.onRun(code, meta)
          return value
        })
        .catch(err => this.error(err))
    }

    getScripts(code, temp) {
      let scripts = [];
      let opt = new Option;                                           // Temp element
      opt.innerHTML=code;                                             // Create a real elements from the string

      for(code=opt.getElementsByTagName('script');temp=code[0];) {    // Loop trough all the scripts
        temp.parentNode.removeChild(temp);                            // Remove each script
        let elm = document.createElement('script');
        elm.innerHTML = temp.innerHTML;
        scripts.push(elm);
      }
      return [scripts, opt.innerHTML.trim()];
    }

    runScripts(code, dest, temp) {
      let opt = new Option;                                           // Temp element
      opt.innerHTML=code;                                             // Create a real elements from the string
      ...
      for(code=opt.getElementsByTagName('script');temp=code[0];) {    // Loop trough all the scripts
        ...
        let elm = document.createElement('script');
        elm.innerHTML = temp.innerHTML;
        dest.parentNode.appendChild(elm);
      }
      return opt.innerHTML.trim();                                    // Return the cleaned HTML
    }
